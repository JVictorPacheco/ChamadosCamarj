...
**Sessão de 2026-07-18 (continuação): Design do T09/F5b (login real Google) adiantado enquanto se aguarda o Client ID da TI.** Criado `.specs/features/fase-6-admin-log/design-t09-google-oauth.md` — pesquisa de documentação atual feita antes de desenhar (Context7: `Google.Apis.Auth` pro backend validar o id_token do Google, `@react-oauth/google` pro botão de login; Microsoft Learn: boas práticas de JWT Bearer no ASP.NET Core). Fluxo: frontend manda id_token do Google → backend valida (`GoogleJsonWebSignature.ValidateAsync`) + confere domínio `@camarj.com.br` de novo no servidor (defesa em profundidade, não confia só na restrição do Google Cloud) → busca na tabela `UsuarioPerfil` (mesma do F5a) → emite JWT próprio. `UsuarioId`/`UsuarioNome`/`perfilRequisitante` client-supplied viram claims do token via `ICurrentUserService`, sem mudar a forma dos Commands/Handlers existentes.

**3 decisões de segurança confirmadas com o usuário:** (1) chave de assinatura do JWT simétrica, não assimétrica — a doc oficial recomenda assimétrica, mas como a mesma API emite e valida o próprio token, simétrica é proporcional aqui; (2) token dura 8-12h sem refresh; (3) **logout automático por 20 minutos de inatividade** (mouse/teclado/clique/scroll), independente da expiração do token — ideia do usuário, pra cobrir o caso de alguém deixar o computador desbloqueado com a aba aberta. Implementação 100% frontend (`useInactivityLogout`), sem custo de backend.

**Ainda não virou `tasks.md`/Execute** — o Design está pronto, mas o Execute de verdade depende do Client ID que a TI ainda não devolveu (o documento já foi entregue). Tasks (quebra atômica) pode ser feita antes disso, já que não depende do valor real do Client ID, só da estrutura.
